...
function decodePng(buf) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    const filterType = uncompressed[rowStart]; // 0 expected for raw unfiltered or we handle type 0
...
function createPaddedIcon(srcImg, targetSize, logoScaleRatio, bgColor = null) {
  ...
  for (let dy = 0; dy < targetLogoSize; dy++) {
    ...
    for (let dx = 0; dx < targetLogoSize; dx++) {
      ...
      if (!bgColor || srcA >= 0.99) {
        // Direct copy / alpha blend over target
...
// 1. Generate 192x192 padded logo (transparent background, logo size ~62% of canvas)
const logo192Buf = createPaddedIcon(srcImg, 192, 0.62);
fs.writeFileSync('public/logo-192.png', logo192Buf);
console.log('Generated public/logo-192.png (62% logo scale)');

// 2. Generate 512x512 padded logo (transparent background, logo size ~62% of canvas)
const logo512Buf = createPaddedIcon(srcImg, 512, 0.62);
fs.writeFileSync('public/logo-512.png', logo512Buf);
console.log('Generated public/logo-512.png (62% logo scale)');
...
// Also replace public/logo.png with the padded version so any direct icon reference has proper margins
