...
export default function KelolaGuruView({ userSession, onRefreshHistory }: KelolaGuruViewProps) {
  ...
  async function loadUsersData() {
    ...
    try {
      ...
      const { data: siswaData, error: siswaError } = await supabase
        .from("siswa").select("*").order("nama", { ascending: true });
      if (siswaError) throw siswaError;
      setStudentsList(siswaData || []);
    } catch (err: any) {
      console.error("Gagal memuat data guru:", err);
      setErrorMsg("Gagal memuat daftar guru.");
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    setErrorMsg("");
    setSuccessMsg("");
    if (!nip) { setErrorMsg("NIP wajib diisi."); return; }
    if (!fullName) { setErrorMsg("Nama Lengkap wajib diisi."); return; }

    const finalEmail = `${nip}@sman19.sch.id`;
    ...
    try {
      const { error: signUpError } = await supabaseAdminAuth.auth.admin.createUser({
        ...
        password: "guru19*",
        email_confirm: true,
        user_metadata: { fullName, role: "guru", nis: null }
      });
      if (signUpError) throw signUpError;
      setSuccessMsg(`Akun guru "${fullName}" berhasil dibuat!`);
      setFullName(""); setNip(""); setIsAddUserOpen(false);
      loadUsersData();
    } catch (err: any) {
      setErrorMsg(err.message || "Gagal membuat akun guru.");
  ...
  const handleBulkDelete = () => {
    ...
    if (ids.length === 0) { alert("Tidak ada akun guru yang dipilih."); return; }
  ...
  const executeBulkDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      showToast(`${ids.length} akun guru berhasil dihapus.`);
  ...
  const handleEditAccount = async (e: React.FormEvent) => {
    ...
    setIsEditing(true);
    try {
      const fullEmail = `${editEmail.trim()}@sman19.sch.id`;
      const updates: any = {};
      if (fullEmail !== editingProfile.email) updates.email = fullEmail;
      ...
      if (editNama !== editingProfile.nama || fullEmail !== editingProfile.email) {
        const { error: profileErr } = await supabase
          .from("profiles").update({ nama: editNama, email: fullEmail }).eq("id", editingProfile.id);
  ...
  const downloadUserTemplate = () => {
    try {
      const data = [["Nama Lengkap", "NIP Guru"], ["Hendra Wijaya, M.Si.", "19761102"]];
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const nipVal = String(row[1] || "").trim();
          if (!name || !nipVal) continue;
          try {
            const { error } = await supabaseAdminAuth.auth.admin.createUser({
              email: `${nipVal}@sman19.sch.id`, password: "guru19*", email_confirm: true,
              user_metadata: { fullName: name, role: "guru", nis: null }
            });
            if (error) throw error; added++; await new Promise(r => setTimeout(r, 200));
          } catch { fail++; }
        }
        if (added > 0) { setSuccessMsg(`${added} akun guru berhasil diimpor!${fail > 0 ? ` (${fail} gagal)` : ""}`); setIsImportUserOpen(false); loadUsersData(); }
  ...
  return (
    <div className="space-y-6 pb-8">
      <input type="file" id="guru-photo-upload-input" accept="image/*" className="hidden" onChange={handleProfilePhotoChange} />
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 flex-1">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" placeholder="Cari nama guru atau NIP..." value={searchQuery}
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="bg-rose-50 border border-rose-100 rounded-3xl p-4 flex items-center justify-between shadow-lg shadow-rose-900/5">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-xs font-black text-rose-950 uppercase tracking-wider">{selectedIds.length} Guru Terpilih</span>
              </div>
        ...
        <motion.div key="table" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-brand-100/40">
                {isLoading ? (
                  ...
                  <tr><td colSpan={6} className="py-12 text-center text-slate-400 text-xs font-bold">Tidak ada data guru.</td></tr>
                ) : (
                      ...
                      <tr key={p.id} className={`hover:bg-brand-50/20 transition-colors ${isSelected ? "bg-brand-50/40" : ""}`}>
                        ...
                        <td className="py-4 px-4">
                          <div className="relative group">
                            ...
                            <label htmlFor="guru-photo-upload-input"
                        ...
                        <td className="py-4 px-6 font-mono font-bold text-sm text-brand-900">{p.email.split("@")[0]}</td>
          ...
          <div className="bg-brand-50/30 p-4 border-t border-brand-100 text-sm text-brand-500 font-bold flex flex-col sm:flex-row items-center justify-between gap-3">
            <span className="whitespace-nowrap tabular-nums">
              Menampilkan {filteredProfiles.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}–{Math.min(currentPage * itemsPerPage, filteredProfiles.length)} dari {filteredProfiles.length} guru
            </span>
            {totalPages > 1 && (
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
                ...
                <div className="flex justify-between items-center border-b pb-3 border-brand-50">
                  <h3 className="text-base font-extrabold text-brand-950 flex items-center gap-2">
                    <GraduationCap className="w-5 h-5 text-amber-600" />Tambah Guru Baru
                  </h3>
                ...
                <form onSubmit={handleCreateUser} className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-xs font-black text-brand-900 uppercase block">Nama Lengkap & Gelar</label>
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="text" required value={nip} onChange={(e) => setNip(e.target.value)}
                  ...
                  <div className="text-[10px] font-bold text-brand-500 bg-brand-50/60 border border-brand-100/50 p-3 rounded-2xl space-y-1 leading-relaxed">
                    <div><strong className="text-brand-900 font-extrabold">Username (Login):</strong> {nip || "[NIP]"}</div>
                    <div><strong className="text-brand-900 font-extrabold">Password:</strong> guru19*</div>
                  </div>
          ...
          {editingProfile && (
            <div className="fixed inset-0 bg-brand-950/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
                ...
                className="bg-white rounded-3xl p-6 w-full max-w-md border border-brand-100 shadow-2xl space-y-4 relative z-10">
                <div className="flex justify-between items-center border-b pb-3 border-brand-50">
                  <h3 className="text-base font-extrabold text-brand-950 flex items-center gap-2"><Pencil className="w-5 h-5 text-brand-600" />Edit Guru</h3>
                ...
                <form onSubmit={handleEditAccount} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-black text-brand-900 uppercase block">Username (Login)</label>
                    <div className="relative">
                      <Mail className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                      <input type="text" required value={editEmail} readOnly
                        className="w-full border border-brand-100 rounded-xl py-2.5 pl-10 pr-4 text-sm font-semibold focus:ring-1 focus:ring-brand-500 outline-none text-brand-900 bg-brand-50/20 opacity-75 cursor-not-allowed" />
                    </div>
          ...
          {isImportUserOpen && (
            <div className="fixed inset-0 bg-brand-950/65 backdrop-blur-xs flex items-center justify-center z-50 p-4">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
                ...
                <div className="flex justify-between items-center border-b pb-3 border-brand-50">
                  <h3 className="text-base font-extrabold text-brand-950 flex items-center gap-2">
                    <FileSpreadsheet className="w-5 h-5 text-emerald-600" />Impor Guru dari Excel
                  </h3>
                ...
                <div className="space-y-4">
                  <p className="text-xs text-brand-500 leading-relaxed font-medium">
                    Format: Kolom A = Nama Lengkap, Kolom B = NIP Guru.
                  </p>
      ...
      <ConfirmationModal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={executeDeleteUser}
        title="Hapus Akun Guru?" message={`Yakin ingin menghapus "${deleteTarget?.email}"?`}
      ...
      <ConfirmationModal isOpen={isBulkDeleteConfirm} onClose={() => setIsBulkDeleteConfirm(false)} onConfirm={executeBulkDelete}
        title="Hapus Massal Guru?" message={`Yakin ingin menghapus ${selectedIds.length} akun guru?`}
