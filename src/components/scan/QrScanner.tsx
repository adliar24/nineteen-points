...
export default function QrScanner({
  ...
  const handleDecoded = useCallback(async (decodedText: string) => {
    ...
    const now = Date.now();
    if (isProcessingRef.current || now < cooldownUntilRef.current) {
      return;
    }
      ...
      <div className="absolute top-0 inset-x-0 z-30 bg-gradient-to-b from-black/95 via-black/75 to-transparent pt-safe pt-3 sm:pt-6 pb-3 sm:pb-6 px-3.5 sm:px-8">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-900/50 flex-shrink-0">
