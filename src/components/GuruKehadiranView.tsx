...
export default function GuruKehadiranView({ userSession }: GuruKehadiranViewProps) {
  ...
  const todaySchedules = useMemo(() => mergeSchedules(schedules.filter(s => s.hari === todayDayName)), [schedules, todayDayName]);

  // Calculate today's attendance stats & percentage
  const todayStats = useMemo(() => {
    ...
    return { total, hadirCount, absentCount, percentage };
  ...
  // Merge history records so 2 JP in the same class & date don't duplicate in rekap
  const mergedHistory = useMemo(() => {
    ...
    for (const record of history) {
      ...
      const key = `${record.tanggal}_${scheduleInfo.mata_pelajaran}_${scheduleInfo.kelas}_${record.status}`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-brand-100 shadow-xl shadow-brand-900/5 space-y-5">
            ...
            {/* PERCENTAGE & STATS CARD HARI INI */}
            {todaySchedules.length > 0 && (
              <div className="grid grid-cols-3 gap-2.5 pb-2">
                <div className="bg-brand-50/60 border border-brand-100 p-3 rounded-2xl flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-xl bg-brand-100 text-brand-700 flex items-center justify-center font-bold flex-shrink-0">
                    <BookOpen className="w-3.5 h-3.5" />
                  </div>
                  <div>
                    <p className="text-[9.5px] text-brand-500 font-bold uppercase tracking-wider leading-none">Total KBM</p>
                    <p className="text-xs font-black text-brand-950 mt-1 leading-none">{todayStats.total} Kelas</p>
                  </div>
                </div>

                <div className="bg-emerald-50/40 border border-emerald-100 p-3 rounded-2xl flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[9.5px] text-emerald-600 font-bold uppercase tracking-wider leading-none">Tercatat</p>
                    <p className="text-xs font-black text-emerald-900 mt-1 leading-none">{todayStats.hadirCount} Kelas</p>
                  </div>
                </div>

                <div className="bg-gradient-to-br from-indigo-50 to-brand-50 border border-indigo-100 p-3 rounded-2xl flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-[9.5px] text-indigo-600 font-bold uppercase tracking-wider leading-none">Persentase</p>
                    <p className="text-xs font-black text-indigo-950 mt-1 leading-none">{todayStats.percentage}%</p>
                  </div>
                </div>
              </div>
            )}
                ...
                {todaySchedules.map((sched, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ type: "spring", stiffness: 90, damping: 14, delay: index * 0.04 }}
                      className={`p-5 rounded-2xl border transition-all duration-200 flex items-center justify-between gap-4 relative overflow-hidden ${
                        isCheckedIn 
                        ...
                        <button
                          ...
                          className={`py-2 px-4 font-extrabold text-xs rounded-xl shadow-sm border transition-all flex items-center gap-1.5 z-10 ${
                            active
                              ...
                              : "bg-slate-100 text-slate-400 border-slate-200/60 cursor-not-allowed shadow-none opacity-80"
                          }`}
                        >
                          {active ? <LogIn className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
                          {active ? "Absen" : "Belum Waktunya"}
                        </button>
                      )}
                    </motion.div>
                  );
