...
export default function KelolaPenggunaView({ userSession, onRefreshHistory }: KelolaPenggunaViewProps) {
  ...
  const handleForceDeleteUser = async (id: string, email: string) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus akun user (${email})?\nPERHATIAN: Akun login auth dan profilnya akan dihapus.`)) {
      return;
    }
    ...
    try {
      ...
      showToast(`Profil akun ${email} berhasil dihapus.`);
      loadUsersData();
    } catch (err: any) {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          try {
            // Call Supabase Auth signup via admin client
            const { error: signUpError } = await supabaseAdminAuth.auth.signUp({
              email: emailVal,
            ...
            // Tiny delay to prevent rate limit throttling
    ...
    <div className="space-y-6">
      ...
      {dbTab === "students" ? (
        <div className="animate-fade-in">
          {/* Reuse the existing KelolaSiswaView directly inside the Admin Database workspace */}
                    ...
                    {[...Array(5)].map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        <td className="py-4 px-6">
                          <div className="h-5 bg-brand-100/60 rounded-md w-36"></div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="h-4.5 bg-brand-50 rounded-md w-48"></div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="h-6 bg-brand-100/55 rounded-lg w-16"></div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="h-5 bg-brand-50 rounded-md w-20"></div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="h-4 bg-brand-50 rounded-md w-24"></div>
                        </td>
                        <td className="py-4 px-6 text-right">
                          <div className="h-7 w-7 bg-brand-100/50 rounded-xl ml-auto"></div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-brand-50 text-brand-900 text-sm font-semibold">
                    {profiles.length > 0 ? (
                            ...
                            <td className="py-4 px-6 text-right">
                              {!isSuper && (
                                <button
                                  ...
                                  className="text-rose-500 hover:text-rose-700 p-1.5 hover:bg-rose-50 rounded-xl transition-all cursor-pointer inline-flex items-center"
                                  title="Hapus Profil"
                                >
                      ...
                      <tr>
                        <td colSpan={6} className="py-12 text-center text-brand-400 font-bold">
